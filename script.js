/* ========================================================== */
/* ClubHub - JS (fixed & defensive listeners)
   - restores quick accounts, fixes login redirect, modal visibility,
     theme toggle icons, mobile menu, duplicate ID problems, calendar nav
*/
...
function renderApp() {
  ...
  if (appState.user.role === 'Creator') {
    renderCreatorDashboard(); showPage('creator-dashboard');
  } else if (appState.user.role === 'Admin') {
    renderAdminDashboard(); showPage('admin-dashboard');
  } else {
    renderMemberDashboard(); showPage('member-dashboard');
  }
}
...
function setupPaymentFields() {
  ...
  const cardFields = $('card-fields'); const gcashFields = $('gcash-fields'); const paypalFields = $('paypal-fields'); const paymayaFields = $('paymaya-fields');
  paymentMethodSelect.addEventListener('change', function() {
    const method = this.value;
    cardFields && cardFields.classList.toggle('hidden', method !== 'card');
    gcashFields && gcashFields.classList.toggle('hidden', method !== 'gcash');
    paypalFields && paypalFields.classList.toggle('hidden', method !== 'paypal');
    paymayaFields && paymayaFields.classList.toggle('hidden', method !== 'paymaya');
  });
}

function processPayment() {
  const methodEl = $('payment-method'); const amountEl = $('payment-amount');
  const method = methodEl ? methodEl.value : '';
  const amount = amountEl ? amountEl.value : '';
  if (!method || !amount) { alert('Please select payment method and amount.'); return; }
...
function setupEventListeners() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (target && target.id === 'create-club-btn') showEventModal();
...
/* ========================================================== */
/* History Management & Page Restoration
/* ========================================================== */
const pageHistory = [];
...
/* ========================================================== */
/* Updated showPage function with history support
/* ========================================================== */
...
/* ========================================================== */
/* Updated renderApp function with history support
/* ========================================================== */
function renderApp() {
  // hide nav links
  document.querySelectorAll('.nav-link').forEach(link => link.classList.add('hidden'));
...
/* ========================================================== */
/* Updated setupEventListeners with back button support
/* ========================================================== */
function setupEventListeners() {
...
/* ========================================================== */
// Update the DOMContentLoaded event listener to initialize navigation
document.addEventListener('DOMContentLoaded', () => {
  // restore session if present
  const stored = localStorage.getItem('clubhub_user');
  if (stored) {
